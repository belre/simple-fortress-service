...
import { generateMockData, toPublicId, traverse } from "@/mock/path-resolver.mock";
...
export class SimplePathResolverService implements IIndexCollector {
    ...
    async resolve(pathId: string | null, cursor: string | null, childLimit?: number) : Promise<IndexCollectionResolveResult> {
        ...
        const fetched = await fetch(`api/mock/${pathId}?${cursor ? `cursor=${cursor}` : ''}${childLimit ? `&limit=${childLimit}` : ''}`, {
            method: 'GET'
        })
        if(!fetched.ok) {
            return {
        ...
        const context = (await fetched.json()) as IndexCollectionResolveResult
        return context
    }
}
